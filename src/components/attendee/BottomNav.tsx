...
export function BottomNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-surface md:left-1/2 md:max-w-lg md:-translate-x-1/2 md:rounded-t-xl md:border-x md:shadow-lg"
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <li key={href} className="flex-1">
              <Link
                href={href}
                className={`flex min-h-[52px] flex-col items-center justify-center gap-0.5 rounded-lg px-1 py-1 text-xs font-medium transition-colors ${
                  active
                    ? "text-primary"
